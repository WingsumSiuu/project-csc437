...
router.put("/:userid", (req, res) => {
  ...
  import_user_svc.default.update(userid, editedUser).then((user) => res.json(user)).catch((err) => res.status(404).send(err));
});
router.post("/", (req, res) => {
...
router.delete("/:userid", (req, res) => {
  const { userid } = req.params;
  import_user_svc.default.remove(userid).then(() => res.status(204).end()).catch((err) => res.status(404).send(err));
});
